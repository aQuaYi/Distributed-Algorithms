...
func startNewElection(rf *Raft, null interface{}) fsmState {
	...
	for server := range rf.peers {
		...
		go func(server int, replyChan chan *RequestVoteReply) {
			...
			if !ok {
				debugPrintf("%s 无法获取 R%d 对 %s 的反馈", rf, server, args)
				return
			}
	...
	go func(replyChan chan *RequestVoteReply) {
		...
		for {
			...
			case <-rf.endElectionChan:
				...
				debugPrintf("%s 收到 election timeout 的信号，停止统计投票的工作: %v", rf, rf.state)
				return
...
func heartbeating(rf *Raft) {
	hbPeriod := time.Duration(HBInterval) * time.Millisecond
...
func oneHearteat(rf *Raft) {
	...
	for server := range rf.peers {
		...
		go func(server int, args *AppendEntriesArgs) {
			...
			if !ok {
				debugPrintf("%s 无法获取 R%d 对 %s 的回复", rf, server, args)
				return
			}
